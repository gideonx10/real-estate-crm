...
export function BuilderCard({ builder, onDelete }) {
  ...
  return (
    <>
      <Card>
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          ...
          <div className="flex gap-2">
            ...
            <Button aria-label="Edit builder" size="icon" variant="secondary" href={`/contacts/builders/${builder.id}`}>
              <Edit3 className="text-zinc-500" size={17} />
            </Button>
            <Button aria-label="Delete builder" size="icon" variant="danger" onClick={() => setConfirmOpen(true)}>
              <Trash2 size={17} />
            </Button>
          </div>
        </div>
      ...
      <ConfirmDialog
        open={confirmOpen}
        message={`Are you sure you want to delete "${builder.full_name}"?`}
        onConfirm={() => { setConfirmOpen(false); onDelete?.(builder.id); }}
